import asyncio
from typing import List, Dict, Optional, Any
import logging
from dataclasses import dataclass
import json

@dataclass
class RAGResult:
    """Result from RAG query"""
    answer: str
    sources: List[Dict]
    confidence: float
    query: str
    context_used: str

class RAGEngine:
    """Improved RAG engine for querying research papers"""
    
    def __init__(self, vector_store: 'VectorStore', config: Dict):
        self.vector_store = vector_store
        self.config = config
        self.max_context_length = config.get('max_context_length', 4000)
        self.max_retrieved_chunks = config.get('max_retrieved_chunks', 5)
        self.similarity_threshold = config.get('similarity_threshold', 0.3)  # Lower threshold
    
    async def query(
        self, 
        question: str, 
        context_filter: Optional[Dict] = None,
        max_results: int = None
    ) -> RAGResult:
        """Query the research paper database"""
        
        max_results = max_results or self.max_retrieved_chunks
        
        print(f"Searching for: {question}")
        print(f"Using similarity threshold: {self.similarity_threshold}")
        
        # 1. Retrieve relevant chunks
        search_results = await self.vector_store.search(
            query=question,
            max_results=max_results * 2,  # Get more results initially
            similarity_threshold=self.similarity_threshold
        )
        
        print(f"Found {len(search_results)} relevant chunks")
        
        if not search_results:
            # Try with an even lower threshold
            search_results = await self.vector_store.search(
                query=question,
                max_results=max_results * 2,
                similarity_threshold=0.1
            )
            print(f"Retry with lower threshold found {len(search_results)} chunks")
        
        if not search_results:
            return RAGResult(
                answer="I couldn't find any relevant research papers to answer your question. The vector database might be empty or the query might not match any stored content.",
                sources=[],
                confidence=0.0,
                query=question,
                context_used=""
            )
        
        # 2. Prepare context and sources
        context_parts = []
        sources = []
        current_length = 0
        
        # Sort results by similarity
        search_results.sort(key=lambda x: x['similarity'], reverse=True)
        
        for result in search_results[:max_results]:
            # Add source information
            metadata = result['metadata']
            source_info = {
                'title': metadata.get('title', 'Unknown Title'),
                'authors': json.loads(metadata.get('authors', '[]')) if metadata.get('authors') else [],
                'paper_id': metadata.get('paper_id', ''),
                'similarity': round(result['similarity'], 3),
                'chunk_type': metadata.get('chunk_type', ''),
                'url': metadata.get('url', ''),
                'categories': json.loads(metadata.get('categories', '[]')) if metadata.get('categories') else []
            }
            sources.append(source_info)
            
            # Add to context if we have space
            chunk_text = result['document']
            chunk_header = f"\n=== {source_info['title']} (Similarity: {source_info['similarity']}) ===\n"
            full_chunk = chunk_header + chunk_text
            
            if current_length + len(full_chunk) < self.max_context_length:
                context_parts.append(full_chunk)
                current_length += len(full_chunk)
        
        context = "\n".join(context_parts)
        
        # 3. Generate answer
        answer = await self._generate_answer(question, context, sources, search_results)
        
        # 4. Calculate confidence
        if search_results:
            avg_similarity = sum(r['similarity'] for r in search_results[:max_results]) / min(len(search_results), max_results)
            confidence = min(avg_similarity, 1.0)
        else:
            confidence = 0.0
        
        return RAGResult(
            answer=answer,
            sources=sources,
            confidence=confidence,
            query=question,
            context_used=context[:500] + "..." if len(context) > 500 else context
        )
    
    async def _generate_answer(
        self, 
        question: str, 
        context: str, 
        sources: List[Dict],
        search_results: List[Dict]
    ) -> str:
        """Generate an improved answer based on the retrieved context"""
        
        if not context or not sources:
            return f"I couldn't find relevant information to answer '{question}'. This could mean:\n1. No papers in the database discuss this topic\n2. The question might need to be rephrased\n3. Try searching for papers on this topic first"
        
        # Extract key information
        unique_papers = {}
        for source in sources:
            paper_id = source['paper_id']
            if paper_id not in unique_papers:
                unique_papers[paper_id] = {
                    'title': source['title'],
                    'authors': source['authors'][:2],  # First 2 authors
                    'similarity': source['similarity'],
                    'categories': source['categories']
                }
        
        # Generate structured answer
        answer_parts = [
            f"Based on {len(search_results)} relevant sections from {len(unique_papers)} research paper(s), here's what I found regarding: '{question}'",
            ""
        ]
        
        # Add key findings
        high_similarity_results = [r for r in search_results if r['similarity'] > 0.5]
        if high_similarity_results:
            answer_parts.append("🔍 **Key Findings:**")
            for i, result in enumerate(high_similarity_results[:3], 1):
                content = result['document'][:200] + "..." if len(result['document']) > 200 else result['document']
                title = result['metadata'].get('title', 'Unknown')
                answer_parts.append(f"{i}. From '{title}': {content}")
            answer_parts.append("")
        
        # Add context summary
        if context:
            # Extract the most relevant sentences
            sentences = context.split('.')
            relevant_sentences = []
            
            question_words = set(question.lower().split())
            for sentence in sentences:
                sentence_words = set(sentence.lower().split())
                overlap = len(question_words.intersection(sentence_words))
                if overlap > 0:
                    relevant_sentences.append((sentence.strip(), overlap))
            
            # Sort by relevance and take top sentences
            relevant_sentences.sort(key=lambda x: x[1], reverse=True)
            
            if relevant_sentences:
                answer_parts.append("📋 **Relevant Information:**")
                for sentence, _ in relevant_sentences[:3]:
                    if len(sentence) > 20:  # Skip very short sentences
                        answer_parts.append(f"• {sentence}.")
                answer_parts.append("")
        
        # Add paper references
        answer_parts.append(f"📚 **Sources ({len(unique_papers)} papers):**")
        for i, (paper_id, info) in enumerate(unique_papers.items(), 1):
            authors_str = ", ".join(info['authors']) if info['authors'] else "Unknown authors"
            categories_str = ", ".join(info['categories'][:2]) if info['categories'] else ""
            category_info = f" [{categories_str}]" if categories_str else ""
            answer_parts.append(f"{i}. {info['title']} - {authors_str}{category_info} (relevance: {info['similarity']:.2f})")
        
        # Add confidence note
        avg_similarity = sum(s['similarity'] for s in sources) / len(sources)
        if avg_similarity < 0.3:
            answer_parts.append("\n⚠️ **Note:** The similarity scores are relatively low, which means the answer might not be highly relevant to your specific question. Consider rephrasing your query or searching for more specific papers.")
        
        return "\n".join(answer_parts)
    
    async def get_paper_summary(self, paper_id: str) -> Optional[Dict]:
        """Get a summary of a specific paper"""
        
        try:
            # Get all chunks for this paper from vector store
            # This is a workaround since we need to access the collection directly
            collection = self.vector_store.collection
            
            results = collection.get(
                where={"paper_id": paper_id},
                include=['documents', 'metadatas']
            )
            
            if not results['documents']:
                return None
            
            # Find the title/abstract chunk
            title_abstract_chunk = None
            content_chunks = []
            metadata = None
            
            for i, doc_metadata in enumerate(results['metadatas']):
                if doc_metadata.get('chunk_type') == 'title_abstract':
                    title_abstract_chunk = results['documents'][i]
                    metadata = doc_metadata
                else:
                    content_chunks.append(results['documents'][i])
            
            summary = {
                'paper_id': paper_id,
                'title_abstract': title_abstract_chunk,
                'content_chunks': len(content_chunks),
                'metadata': metadata if metadata else results['metadatas'][0] if results['metadatas'] else {}
            }
            
            return summary
            
        except Exception as e:
            logging.error(f"Error getting paper summary: {e}")
            return None